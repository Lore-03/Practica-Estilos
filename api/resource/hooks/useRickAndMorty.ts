...
export function useCharacters(filters?: CharacterFilters) {
  ...
  const fetchCharacters = useCallback(async (customFilters?: CharacterFilters) => {
    console.log('🔍 fetchCharacters llamado con filtros:', customFilters || filters);
    return executeRequest(() => rickAndMortyApi.characters.getAll(customFilters || filters));
...
export function useLocations(filters?: LocationFilters) {
  ...
  const fetchLocations = useCallback(async (customFilters?: LocationFilters) => {
    return executeRequest(() => rickAndMortyApi.locations.getAll(customFilters || filters));
  }, [executeRequest, filters]);
  ...
  const fetchLocationsByIds = useCallback(async (ids: number[]) => {
...
export function useEpisodes(filters?: EpisodeFilters) {
  ...
  const fetchEpisodes = useCallback(async (customFilters?: EpisodeFilters) => {
    return executeRequest(() => rickAndMortyApi.episodes.getAll(customFilters || filters));
  }, [executeRequest, filters]);

  const fetchEpisodeById = useCallback(async (id: number) => {
    return executeRequest(() => rickAndMortyApi.episodes.getById(id));
  }, [executeRequest]);

  const fetchEpisodesByIds = useCallback(async (ids: number[]) => {
